...
|------------------------------|---------------------------------------------------|------|-----|
| repo root                    | `/workspace/cytometer`                   | rw   | The cell repo. |
| `~/.claude/projects/<repo>`  | `/home/agent/.claude/projects/-workspace-cytometer` | rw   | Sessions + memory persist across container runs. |
...
|-----------------------------------------|---------------------------------------------------------|---------------|
| `cytometer-agent-home`         | `/home/agent`                                           | The whole agent home dir. Includes `~/.claude/` (OAuth tokens, session metadata) AND `~/.claude.json` (Claude Code's user-config file at the home root — the first-run wizard re-prompts forever if this isn't persisted). |
| `cytometer-agent-venv-queue`   | `/workspace/cytometer/dev-tools/queue/.venv`   | uv project venv for the bundled queue |
...
```bash
docker volume rm cytometer-agent-home
...
docker build -t cytometer-agent:latest dev-tools/agent-container/
...
- The repo is at `/workspace/cytometer`. `git status` works
  as expected.
...
```bash
AGENT_CONTAINER_IMAGE=cytometer-agent:latest \
    dev-tools/agent-container/run.sh
```
