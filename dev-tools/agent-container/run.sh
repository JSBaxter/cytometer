#!/usr/bin/env bash
# Launch the cytometer agent container with the right bind-mounts.
#
...
IMAGE="${AGENT_CONTAINER_IMAGE:-cytometer-agent:latest}"
...
# /workspace/cytometer, so the in-container project dir is
# named `-workspace-cytometer`. Bind-mount the host's
...
CONTAINER_PROJECT_NAME="-workspace-cytometer"
...
# already-built venv).
VENV_VOLUMES=(
    -v cytometer-agent-venv-queue:/workspace/cytometer/dev-tools/queue/.venv
)
...
if [[ -f "$REPO_ROOT/.claude/settings.local.json" ]]; then
    HOST_SETTINGS_MASK+=(-v /dev/null:/workspace/cytometer/.claude/settings.local.json:ro)
fi
...
exec docker run --rm "${TTY_FLAGS[@]}" \
    ...
    -v "$REPO_ROOT:/workspace/cytometer" \
    "${HOST_SETTINGS_MASK[@]}" \
    "${VENV_VOLUMES[@]}" \
    -v cytometer-agent-home:/home/agent \
    -v "$HOST_PROJECT_DIR:/home/agent/.claude/projects/$CONTAINER_PROJECT_NAME" \
    -w /workspace/cytometer \
    "$IMAGE" \
    "$@"
