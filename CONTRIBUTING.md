...
review comment, a **merged PR**, or a manual `workflow_dispatch`. **Only the
operator and the colony bot can trigger a run** — a job-level `if:` plus a
verify step fail closed for anyone else, and repo interaction limits are a
second layer. Claude commits its work under the colony bot App identity and
opens a PR, so the operator stays a real reviewer.

**The run loop is a human-paced ratchet.** At most one agent run executes at
a time (`concurrency`), and the agent produces **at most one PR per run, then
stops**. While an agent PR (head `claude/*`) is open awaiting review, new
issues are **deferred** — they wait in the backlog rather than starting a
second PR. Merging a PR wakes the agent for the **post-merge cycle**: it
checks `CEREMONIES.md` (by the merged PR number), and if no ceremony is due it
pulls the next item — open `claude-build` issues are the inbox, scoped into
the `dev-tools/queue`, which is the working backlog. If there is no ceremony
and no work, it exits until the next issue. So: merge → one unit of work →
one PR → your review → merge, repeating until the backlog drains. Operator PR
reviews and `workflow_dispatch` bypass the defer guard (they act on the
pending PR / are an explicit override).

It uses the official `anthropics/claude-code-action@v1` with Claude
**subscription** auth (not API-billed). It needs these repo secrets (set
once per cell, or shared at user level): `CLAUDE_CODE_OAUTH_TOKEN` (from
`claude setup-token` — a ~1-year token; re-run and update the secret when
it expires) and `COLONY_BOT_APP_ID` + `COLONY_BOT_PRIVATE_KEY` (the colony
App used by `dev-tools/agent-bot/`). The App must have `issues:write`,
